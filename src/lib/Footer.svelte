<script lang="ts">
	...
	function phoneLink() {
		return `tel:${footer_phone}`;
	}

	function getEmail() {
		const regex = new RegExp('([\\w\\d._%+-]+)@([\\w\\d-]+)\\.([\\w\\d-]+)?\\.?(\\w+)?');

		let [original, ...matches] = footer_email.match(regex);
		matches = matches.filter((a) => a);
		const name = matches.shift();
		const tld = matches.pop();
		const domain = matches.join('.');

		new EmailObfuscator(email, {
			name,
			domain,
			tld,
			altText: 'Email MAMC'
...
<template>
			...
			<footer>
				<div class="info">
                    ...
                    c-34.123,0-61.884-27.761-61.884-61.884s27.761-61.884,61.884-61.884C247.104,74.736,274.865,102.497,274.865,136.62z"
							...
							<a href={footer_address_link}>{footer_address}</a>
						</li>
						<li>
							<span class="icon">
								<svg
	...
	c0,0,0,0,0.001,0c3.72,0,7.188-1.581,10.305-4.698l2.537-2.537C54.033,45.163,54.383,42.833,53.364,40.908z"
							...
							<a href={phoneLink()}>{footer_phone}</a>
						</li>
						<li>
							<span class="icon">
								<svg
		...
		h56.862L38.07,31.954z"
				...
				<client-only>
					<form name="contact" method="POST" data-netlify="true">
						...
						<label>
							<textarea placeholder="Message" name="message" required maxlength="500" />
						</label>
